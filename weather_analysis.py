...
# Pie chart for average temperature distribution
temp_bins = pd.cut(data['tempavg'], bins=[-10, 0, 10, 20, 30, 40])
temp_distribution = temp_bins.value_counts().sort_index()
axs[0, 0].pie(temp_distribution, labels=temp_distribution.index, autopct='%1.1f%%', startangle=90, colors=plt.cm.Reds(np.linspace(0, 1, len(temp_distribution))))
axs[0, 0].set_title('Temperature Distribution')

# Pie chart for average humidity distribution
humidity_bins = pd.cut(data['humidity avg'], bins=[0, 20, 40, 60, 80, 100])
humidity_distribution = humidity_bins.value_counts().sort_index()
axs[0, 1].pie(humidity_distribution, labels=humidity_distribution.index, autopct='%1.1f%%', startangle=90, colors=plt.cm.Blues(np.linspace(0, 1, len(humidity_distribution))))
axs[0, 1].set_title('Humidity Distribution')

# Pie chart for average wind speed distribution
wind_bins = pd.cut(data['windavg'], bins=[0, 10, 20, 30, 40, 50])
wind_distribution = wind_bins.value_counts().sort_index()
axs[1, 0].pie(wind_distribution, labels=wind_distribution.index, autopct='%1.1f%%', startangle=90, colors=plt.cm.Greens(np.linspace(0, 1, len(wind_distribution))))
axs[1, 0].set_title('Wind Speed Distribution')

# Pie chart for rainfall distribution
rainfall_bins = pd.cut(data['Rainfall'], bins=[0, 10, 20, 30, 40, 50])
rainfall_distribution = rainfall_bins.value_counts().sort_index()
axs[1, 1].pie(rainfall_distribution, labels=rainfall_distribution.index, autopct='%1.1f%%', startangle=90, colors=plt.cm.Purples(np.linspace(0, 1, len(rainfall_distribution))))
axs[1, 1].set_title('Rainfall Distribution')

plt.tight_layout()
plt.savefig('weather_distribution_pie_charts.png')
plt.show()
